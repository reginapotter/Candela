...
use Candela\Blog\Block\Layout\MobileWrapper;
use Candela\Blog\Block\Layout\WidgetWrapper;
use Candela\Blog\Exceptions\LayoutGenerationException as LayoutGenerationException;
...
class Generator implements GeneratorInterface
{
        ...
        foreach (ConfigFactory::DI_NAMES_MAP as $referenceBlockName => $sectionName) {
            ...
            if (!empty($sectionBlocks)) {
                ...
                $referenceSection->addAttribute(
                    'name',
                    sprintf('candela_blog.layout.%s', $referenceBlockName)
                );
                ...
                if (in_array($layoutConfig->getConfigIdentifier(), [self::MOBILE_LIST, self::MOBILE_POST])
                    ...
                    $header->addAttribute('name', 'candela_blog.layout.header');
    ...
    private function generateMobileWrapper(SimpleXMLElement $parentElement, bool $rightSide): SimpleXMLElement
    {
        ...
        $mobileWrapper->addAttribute('template', 'Candela_Blog::list/mobile_wrapper.phtml');
    ...
    private function getElementWrapper(SimpleXMLElement $target, string $nameInLayout): SimpleXMLElement
    {
        ...
        $wrapper->addAttribute('template', 'Candela_Blog::widget_wrapper.phtml');
