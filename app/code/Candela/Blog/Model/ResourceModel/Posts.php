...
use Candela\Blog\Helper\Data as BlogHelper;
use Candela\Blog\Model\Blog\Registry;
use Candela\Blog\Model\ImageProcessor;
use Candela\Blog\Model\Posts as PostModel;
use Candela\Blog\Model\ResourceModel\Posts\Save\SavePartInterface;
use Candela\Blog\Model\ResourceModel\Posts\Save\SavePartProcessorsPool;
use Candela\Blog\Model\ResourceModel\Traits\ResourceModelTrait;
...
class Posts extends AbstractDb
{
    ...
    public const TABLE_NAME = 'candela_blog_posts';

    public const STORE_TABLE_NAME = 'candela_blog_posts_store';

    public const POSTS_TAGS_RELATION_TABLE = 'candela_blog_posts_tag';

    public const POSTS_STORE_TABLE = 'candela_blog_posts_store';
    ...
    /**
     * @var \Candela\Blog\Api\Data\TagInterface
     */
    private $tagModel;
    ...
    private function loadTags(PostInterface $postModel)
    {
        ...
        if ($postId) {
            $select = $this->getConnection()->select()
                ...
                ->joinLeft(
                    ['tags' => $this->getTable('candela_blog_tags_store')],
    ...
    private function loadCategories(PostInterface $postModel)
    {
        ...
        if ($postId) {
            $select = $this->getConnection()->select()
                ->from(
                    ['posts_cats' => $this->getTable('candela_blog_posts_category')],
                    ['category_id']
                )->joinLeft(
                    ['cats' => $this->getTable('candela_blog_categories')],
    ...
    private function validateUrlKey(PostInterface $post)
    {
        ...
        $select = $connection->select()->from(
            ...
            ['store' => $this->getTable('candela_blog_posts_store')],
    ...
    private function saveStores(PostInterface $post, AdapterInterface $connection)
    {
        ...
        if (!empty($stores)) {
            ...
            $connection->delete($this->getTable('candela_blog_posts_store'), $condition);
            ...
            foreach ($stores as $storeId) {
                ...
                $connection->insert($this->getTable('candela_blog_posts_store'), $storeInsert);
            }
        }
    ...
    private function saveTags(PostInterface $post, AdapterInterface $connection)
    {
        ...
        $condition = [PostInterface::POST_ID . ' = ?' => $post->getPostId()];
        $connection->delete($this->getTable('candela_blog_posts_tag'), $condition);
        if (!empty($tags)) {
            ...
            foreach ($tagsArray as $tag) {
                ...
                $connection->insert($this->getTable('candela_blog_posts_tag'), $tagInsert);
            }
        }
    ...
    private function saveCategories(PostInterface $post, AdapterInterface $connection)
    {
        ...
        $condition = [PostInterface::POST_ID . ' = ?' => $post->getPostId()];
        $connection->delete($this->getTable('candela_blog_posts_category'), $condition);
        if (!empty($categories)) {
            $insertedIds = [];
            ...
            foreach ($categories as $catId) {
                ...
                $connection->insert($this->getTable('candela_blog_posts_category'), $insert);
            }
        }
