...
use Candela\Blog\Model\ResourceModel\Posts as PostResource;
...
class Collection extends \Candela\Blog\Model\ResourceModel\Abstracts\Collection
{
    ...
    protected function addCategories(): self
    {
        $this->getSelect()
            ->joinLeft(
                ['categories' => $this->getTable('candela_blog_posts_category')],
    ...
    public function addTagFilter($tagIds): self
    {
        ...
        $this->getSelect()
            ->joinLeft(
                ['tags' => $this->getTable('candela_blog_posts_tag')],
    ...
    private function addLinkedData(string $linkedTable, string $linkedField, string $fieldName): void
    {
        ...
        if (!empty($postId)) {
            ...
            $select = $connection->select()
                ->from($this->getTable('candela_blog_posts_' . $linkedTable))->where($inCond);
    ...
    public function addAuthorFilter(?array $authorIds): self
    {
        ...
        $this->getSelect()
            ->join(['author' => $this->getTable('candela_blog_author')], 'author.author_id = main_table.author_id', [])
