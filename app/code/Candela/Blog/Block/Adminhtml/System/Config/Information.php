...
namespace Candela\Blog\Block\Adminhtml\System\Config;
...
class Information extends \Magento\Config\Block\System\Config\Form\Fieldset
{
     ...
     * @var string
     */
    private $userGuide = 'https://candela.com/docs/doku.php?id=magento_2:blog_pro';
    ...
    public function render(AbstractElement $element)
    {
        ...
        $this->setContent(__('Please update Candela Base module. Re-upload it and replace all the files.'));

        $this->_eventManager->dispatch(
            'candela_base_add_information_content',
            ['block' => $this]
        );
        ...
        $html = str_replace(
            'candela_information]" type="hidden" value="0"',
            'candela_information]" type="hidden" value="1"',
            $html
        );
    ...
    public function getAdditionalModuleContent()
    {
        ...
        if ($this->moduleManager->isEnabled('Magento_PageBuilder')
            && !$this->moduleManager->isEnabled('Candela_BlogPageBuilder')
        ) {
            $message = [
                'type' => 'message-notice',
                'text' => __('Enable blogpagebuilder module to activate PageBuilder and Blog Pro integration. '
                    . 'Please, run the following command in the SSH: composer require candela/blog-page-builder')
            ];
